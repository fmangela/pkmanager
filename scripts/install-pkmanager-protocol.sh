...
# Self-localize: this script may live in <repo>/scripts/ with launcher as a
# sibling, or may be a standalone browser download with the launcher embedded
# below the __PKMANAGER_LAUNCHER_SH__ marker.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SOURCE_LAUNCHER="$SCRIPT_DIR/pkmanager-launcher.sh"

extract_embedded_launcher() {
    local target="$1"
    if ! grep -q '^__PKMANAGER_LAUNCHER_SH__$' "$0"; then
        return 1
    fi
    awk '
        found { print }
        /^__PKMANAGER_LAUNCHER_SH__$/ { found = 1 }
    ' "$0" > "$target"
    [ -s "$target" ]
}
...
if [ -f "$SOURCE_LAUNCHER" ]; then
    ...
    info "Launcher source not found next to installer; using embedded launcher payload."
    if ! extract_embedded_launcher "$LAUNCHER_SH"; then
        err "pkmanager-launcher.sh was not found next to this installer and no embedded payload was present."
        info "Expected sibling: $SOURCE_LAUNCHER"
        exit 1
    fi
fi
