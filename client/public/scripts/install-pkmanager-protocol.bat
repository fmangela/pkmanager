...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$batPath='%~f0'; $lines = Get-Content -LiteralPath $batPath -Encoding UTF8; $start = [Array]::IndexOf($lines, 'REM @@LAUNCHER_START@@'); $end = [Array]::IndexOf($lines, 'REM @@LAUNCHER_END@@'); if ($start -lt 0 -or $end -le $start) { throw 'launcher markers not found in ' + $batPath }; $body = $lines[($start + 1)..($end - 1)] | ForEach-Object { $_ -replace '^REM ?', '' }; $encoding = New-Object System.Text.UTF8Encoding $true; [System.IO.File]::WriteAllLines('%PS1%', $body, $encoding)"
...
REM             }
REM         } catch {
REM             # ignore search failures on inaccessible roots
REM         }
REM     }
REM 
REM     if ($fallbackPath) {
