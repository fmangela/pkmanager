...
function getOriginMarkMap(t: (key: string, defaultValue?: string) => string): Record<number, { label: string; color: string }> {
  return {
    0: { label: t('cosmetic.originNone', '无'), color: '#d9d9d9' },
    1: { label: 'Game Boy', color: '#5b8c5a' },
    2: { label: 'GO', color: '#5b9bd5' },
    3: { label: t('cosmetic.originPentagon', '五角形 (Gen6)'), color: '#1890ff' },
    4: { label: t('cosmetic.originClover', '三叶草 (Gen7)'), color: '#eb2f96' },
    5: { label: t('cosmetic.originGalar', '伽勒尔 (Gen8 SWSH)'), color: '#722ed1' },
    6: { label: t('cosmetic.originTrio', 'Trio (Gen8 BDSP)'), color: '#fa8c16' },
    7: { label: t('cosmetic.originHisui', '洗翠 (Gen8 LA)'), color: '#13c2c2' },
    8: { label: t('cosmetic.originPaldea', '帕底亚 (Gen9 SV)'), color: '#f5222d' },
  };
}

const CosmeticTab: React.FC<Props> = ({ pokemon, generation, onChange }) => {
  const { t } = useTranslation('editor');
  const originMarkMap = getOriginMarkMap((key, defaultValue) => t(key, { defaultValue }));
  const g = generation;
  const ch = () => onChange?.();
  ...
  return (
    <div>
      ...
      <div style={sectionStyle}>
        <div style={sectionTitle}>{t('cosmetic.markings', '标记 (Markings)')}</div>
        <Space size="small">
          {MARK_SYMBOLS.map((sym, i) => {
            ...
            return (
              <span
                ...
                title={`${sym} — ${cur === 0 ? t('cosmetic.markingOff', '关') : cur === 1 ? t('cosmetic.markingBlue', '蓝色标记') : t('cosmetic.markingRed', '红色标记')}`}
              >
                {sym}
              </span>
            );
        ...
        <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 6 }}>
          {isGen7Plus
            ? t('cosmetic.markingCycleGen7', '点击循环：关 → 蓝 → 红 → 关')
            : t('cosmetic.markingCycleLegacy', '点击切换：关 ↔ 蓝')}
        </div>
      </div>
        ...
        <div style={sectionStyle}>
          ...
          {pokemon.species === 327 && (
            <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 8 }}>
              {t('cosmetic.spindaContestHint', '注意：晃晃斑的选美属性独立于斑点图案，斑点由 PID 决定（见下方）。')}
            </div>
          )}
        </div>
      )}
      ...
      {isGen6Plus && (
        <div style={sectionStyle}>
          <div style={sectionTitle}>{t('cosmetic.originMark', '来源标记 (Origin Mark)')}</div>
          <OriginMarkDisplay
            mark={pokemon.originMark}
            label={t('cosmetic.originMarkReadonly', '只读 — 由来源游戏版本决定')}
            originMarkMap={originMarkMap}
          />
        </div>
      )}
      ...
      {pokemon.species === 327 && pokemon.pid != null && (
        <div style={sectionStyle}>
          <div style={sectionTitle}>
            {t('cosmetic.spindaSpots', '晃晃斑斑点 (Spinda Spots)')}
            <span style={{ fontWeight: 400, fontSize: 11, color: '#8c8c8c', marginLeft: 8 }}>
              {t('cosmetic.spindaUniqueHint', '由 PID 唯一决定，每个晃晃斑都独一无二')}
            </span>
          </div>
...
// ═══════════════════════════════════════════════
const OriginMarkDisplay: React.FC<{
  mark?: number;
  label: string;
  originMarkMap: Record<number, { label: string; color: string }>;
}> = ({ mark, label, originMarkMap }) => {
  const info = originMarkMap[mark ?? 0] || originMarkMap[0];
...
const SpindaCanvas: React.FC<{ pid: number }> = ({ pid }) => {
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      ...
      ctx.fillText(t('cosmetic.spriteLoadFailed', '精灵图加载失败'), DISPLAY / 2, DISPLAY / 2 + 4);
      setStatus('error');
    };
  }, [pid]);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      />
      <div style={{ fontSize: 11, color: '#8c8c8c', marginTop: 6 }}>
        {status === 'loading' && t('cosmetic.spriteLoading', '加载精灵图中...')}
        {status === 'ready' &&
          `${t('cosmetic.spotCoordinates', '斑点坐标')}: (${spots.map(s => `${s.x},${s.y}`).join(') (')})`}
        {status === 'error' && t('cosmetic.spriteLoadFailedHint', '精灵图加载失败，斑点坐标仍如上所示')}
      </div>
      <code style={{
